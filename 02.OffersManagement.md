...
This Endpoint is to be used whenever an FSP wants to update the status of its offers associated with a single order batch.
...
|---|---|
|mrid|The mrid of the order batch, for which the offers' status should be updated|
...
|---|---|---|
|fields (*string*)|List of field names to return in the response||
...
## Retrieve balance mechanism offers' list by criteria
This Endpoint is to be used whenever an FSP wants to retrieve all of its offers related to balance mechanism, that meet several criteria.
...
**Requested body schema of a** `BMOffer`:

|Field|Description |Allowable responses/examples|
|---|---|---|
|mrid (*string*)|Universally unique id for the offer||
|businessType (*string*)|Nature of the product (only "Balance Mechanism")|enum: `'Balance Mechanism'`|
|measureUnitQuantity (*string*)|Unit of measurement for flexibility available|enum: `'MW'`|
|measureUnitEnergyQuantity (*string*)|Unit of measurement for the energy (utilisation component of the flexibility available|enum: `'MWh'`|
|pricePeriodUnit (*string*)|For what time unit the price is defined per measureUnitQuantity and measureUnitEnergyQuantity|enum: `'Hr'`|
|currency (*string*)|Currency used|enum: `'GBP'`|
...
|flexQtyConvention (*string*)|Flex Quantity Convention|enum: `'Load'`, `'Generation'`|
|reserveObjectLevel (*string*)|Reserve object level|enum: `'Portfolio'`, `'Asset'`|
|direction (*string*)|Direction of the energy flow (convention defined in market rules)|enum: `'UP'`, `'DOWN'`|
|minActivationQuantity (*double*)|minimum volume at which the offer can be accepted for each period|0|
|constraints|||
|periods (*array*)|||
|bmAvailable (*boolean*)|Boolean value to determine whether or not a BMOffer should be created post auction for the BM|enum: `true`, `false`|
|MIL (*number*)|maximum import limit ( MIL) which can be accepted||
|MEL (*number*)|maximum export limit ( MEL) which can be accepted||
|parentOfferID (*string*)|Parent Offer Identifier||
|orderBatchMrid (*string*)|Order batch Identifier||
