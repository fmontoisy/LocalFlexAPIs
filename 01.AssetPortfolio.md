...
This Endpoint is to be used whenever an FSP wants to retrieve a single asset from the LocalFlex platform, based on the filtering from its mrid. The FSP will be able to see to which portfolio the asset is linked and whether or not the asset has been by the system operator and/or the localFlex platform.
